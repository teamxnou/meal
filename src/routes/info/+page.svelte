  ...
  {#if isIdValid}
    <div class="flex grow flex-col">
      <div class="relative flex w-screen grow">
        ...
        <button
          class="absolute bottom-3 left-3 flex h-14 w-14 items-center justify-center rounded-full bg-white/50 shadow-lg"
          on:click={() => scrollCarousel(-1)}
        >
          <ChevronLeft class="h-7 w-7" />
        </button>
        <button
          class="absolute bottom-3 right-3 flex h-14 w-14 items-center justify-center rounded-full bg-white/50 shadow-lg"
          on:click={() => scrollCarousel(1)}
        >
